...
public class ChatListVIewAdapter extends ArrayAdapter<ChatMessageBean> {
    ...
    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            Glide.with(mContext).load(R.drawable.chat_machine_image).asBitmap().centerCrop().into(new BitmapImageViewTarget(viewHolder.head1) {
                @Override
            ...
            Glide.with(mContext).load(R.drawable.chat_left_human).asBitmap().centerCrop().into(new BitmapImageViewTarget(viewHolder.head2) {
                @Override
                protected void setResource(Bitmap resource) {
                    RoundedBitmapDrawable circularBitmapDrawable =
                            RoundedBitmapDrawableFactory.create(mContext.getResources(), resource);
